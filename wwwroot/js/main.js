jQuery(document).ready(function($){
	...
	function createNavigation(steps, n) {
		...
		steps.each(function(index){
			...
			var buildElement = 'span' + stepNumber; 
			if (top.document.getElementById(buildElement)!=null){
				var buildFile = '/signup/sub/' + stepNumber ;
				
				//$(buildElement).load(buildFile);
				
				$.ajax({url:buildFile, success: function(response){
				//$(buildElement).innerHTML=response;
				
				top.document.getElementById(buildElement).innerHTML = response;   	//$(buildElement).load(response);
	...
	function showStep(step, layer) {
		step.addClass('is-selected').removeClass('move-left').addClass('is-visible');
