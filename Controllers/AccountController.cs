...
//
        ...
        [HttpGet]
        [AllowAnonymous]
        public IActionResult SentEmail(string returnUrl = null)
        {
            ViewData["ReturnUrl"] = returnUrl;
            ViewData["User.Identity.Name"] =User.Identity.Name;
            return View();
        }
        ...
        public async Task<IActionResult> Register(RegisterViewModel model, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    return RedirectToAction("",nameof(SentEmail));
                }
                AddErrors(result);
            }
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Logout(int empty=0)
        {
            await _signInManager.SignOutAsync();
            _logger.LogInformation(4, "User logged out.");
            return RedirectToAction(nameof(HomeController.Index), "Home");
        ...
        public async Task<IActionResult> ExternalLoginCallback(string returnUrl = null, string remoteError = null)
        {
            ...
            {   var user = await _userManager.FindByNameAsync(info.Principal.FindFirstValue(ClaimTypes.Email));
                 ...
                 if (!await _userManager.IsPhoneNumberConfirmedAsync(user))
                    {   
                        _logger.LogInformation(5, "External login successful but no phone number assigned redirecting to add phone number.", info.LoginProvider);
            ...
            if (ModelState.IsValid)
            {
               ...
               // user.EmailConfirmed=true;
                if (result.Succeeded)
                {
                    ...
                    ApplicationUser userToUpdate = _context.Users.Where(ci =>  ci.Email==model.Email).First();
